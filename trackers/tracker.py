...
import sys
sys.path.append('../')
from utilities import get_bounding_box_width, get_center_of_bounding_box, get_foot_position
...
class Tracker:
        ...
        for object, object_tracks in tracks.items():
            for frame_num, track in enumerate(object_tracks):
                for track_id, track_info in track.items():
                    ...
                    #Use center for ball, foot position for players/referees
                    if object == 'ball':
    ...
    def interpolate_ball_positions(self,ball_positions):
        #Extract bounding boxes, handling cases where ball detection failed
        ball_positions = [x.get(1,{}).get('bounding_box',[]) for x in ball_positions]
        df_ball_positions = pd.DataFrame(ball_positions, columns = ['x1','y1','x2','y2'])

        #Interpolate missing values and handle edge cases with backward fill
        df_ball_positions = df_ball_positions.interpolate()
        df_ball_positions = df_ball_positions.bfill()
        #Convert interpolated dataframe back to list of dictionaries
    ...
    def get_object_tracks(self, frames, read_from_stub=False, stub_path=None):
        #Load pre-processed tracks from file if available
        ...
        for frame_num, detection in enumerate(detections):
            ...
            cls_names_inv = {v:k for k,v in cls_names.items()} #Create reverse mapping for easier lookup

            #Convert YOLO format to supervision Detection format
            ...
            #Handle ball separately since it doesn't need complex tracking
    ...
    def draw_ellipse(self, frame, bounding_box, color, track_id=None):
        #Draw ellipse at the bottom of bounding box to show player position
        ...
        cv2.ellipse(
            frame,
            center=(x_center, y2),  #center as a tuple of integers
            axes=(int(width), int(0.35*width)),  #axes as a tuple of integers
            angle=0.0,  
            startAngle=-45,   
            endAngle=235, 
            color=color,  
        ...
        #Add player number in a small rectangle
        rectangle_width = 40
        rectangle_height = 20
        ...
        if track_id is not None:
            ...
            if track_id > 99:
                x1_text -= 10
            #Put the track ID number on the rectangle
            cv2.putText(
                    frame,
                    f"{track_id}",
    ...
    def draw_team_ball_control(self, frame, frame_num, team_ball_control):
        ...
        #Calculate ball control percentages up to current frame
    ...
    def draw_annotations(self, video_frames, tracks, team_ball_control):
        ...
        for frame_num, frame in enumerate(video_frames):
            ...
            #Draw player ellipses and possession indicators
            ...
            #Add ball control stats overlay
